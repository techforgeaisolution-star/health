...
def predictparkinsons():
    int_features = [x for x in request.form.values()]
    processed = [np.array(int_features, dtype=float)]
    prediction = parkinsons_predict.predict(processed)
    prob = get_probability(parkinsons_predict, processed)

    result = "positive" if prediction[0] == 1 else "negative"
    display_text = "This person has Parkinson's Disease" if prediction[0] == 1 else "This person does not have Parkinson's Disease"
    ...
    return render_template(
        ...
        disease="Parkinson's",
        factors=[
            {"label": "Jitter", "value": min(100, round(float(int_features[3]) / 0.03316 * 100, 1))},
            {"label": "Shimmer", "value": min(100, round(float(int_features[8]) / 0.11908 * 100, 1))},
            {"label": "HNR (inv)", "value": min(100, round((33.047 - float(int_features[15])) / 33.047 * 100, 1))},
            {"label": "PPE", "value": min(100, round(float(int_features[21]) / 0.527367 * 100, 1))},
        ]
    )
